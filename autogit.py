    ...
    match action:
        case 'config':
            ...
            if not AVAILABLE_CONFIG_OPTIONS.__contains__(command) and not command == '--help':
                    print("Invalid configuration option. Please type `config --help` to see valid options")
            ...
            if len(argv) != 4:
                print("Invalid number of arguments, type `config --help` to see valid options")
                return
